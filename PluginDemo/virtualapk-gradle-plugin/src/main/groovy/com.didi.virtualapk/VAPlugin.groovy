...
//        }
        ...
        /**
         * 如果宿主和插件依赖同一个远程库，如果版本不同，则使用virtualApk.forceUseHostDependences字段判断是否强制使用宿主版本。
         * 实现原理:在宿主编译时收集依赖版本，插件编译时使用configuration.resolutionStrategy遍历插件依赖哪些库。
         * 根据收集到的宿主依赖和插件依赖判断是否存在相同依赖。
         */
    ...
    static class VATaskHookerManager extends TaskHookerManager {
        ...
        void registerTaskHookers() {
            android.applicationVariants.all { ApplicationVariantImpl appVariant ->
                ...
                /**
                 * 查找需要剔除的arr以及需要保留的jar和arr
                 * 需要剔除的arr保存在:vaContext.stripDependencies = stripDependencies
                 * 需要保留的arr保存在:vaContext.retainedAarLibs = retainedAarLibs
                 */
                ...
                /**
                 * 在MergeManifests任务之前，不合并要剔除的arr包中的Manifest
                 * 执行完完后剔除插件中Manifest：icon、label、allowBackup、supportsRtl属性
                 */
                registerTaskHooker(instantiator.newInstance(MergeManifestsHooker, project, appVariant))
                /**
                 * 在mergeJniLibs任务之前，剔除要剔除的arr中的so
                 */
